...
const DashboardPage: React.FC = () => {
	...
	const batteryBgClass =
		battery < 0 ? 'bg-slate-50 text-slate-400' :
			battery < settings.lowBatteryThreshold ? 'bg-red-50 text-red-500' :
				battery < 50 ? 'bg-orange-50 text-orange-500' : 'bg-green-50 text-green-500';
					...
					<div className="col-span-3 grid grid-cols-4 gap-4 h-28 mb-2">
						...
						<div className="bg-white rounded-2xl p-4 shadow-sm border border-slate-100 flex flex-col justify-between">
							<div className="flex justify-between items-start">
								...
								<div className={`p-1.5 rounded-lg ${batteryBgClass}`}>
									<Battery className="w-4 h-4" />
								</div>
							</div>
							<div>
								<div className={`text-2xl font-bold ${batteryColorClass}`}>
									{battery >= 0 ? `${battery}%` : '--%'}
								</div>
						...
						<div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100">
							...
							<div className="space-y-4">
								<div className="p-3 bg-slate-50 rounded-xl">
									...
									<div className="font-bold text-slate-700">Holybro S500</div>
								</div>
								<div className="p-3 bg-slate-50 rounded-xl">
